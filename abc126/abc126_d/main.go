...
func dfs(target, parent, color int, distanceMap []map[int]int, res []int) {
	res[target] = color

	for other, d := range distanceMap[target] {
		if other == parent {
			continue
		}

		dfs(other, target, TernaryInt(d%2 == 0, color, 1-color), distanceMap, res)
	}
}
...
func solve(io *Io, d *Io) {
	...
	for i := 1; i <= N-1; i++ {
		...
		d := w % 2
		distanceMap[u-1][v-1] = d
		distanceMap[v-1][u-1] = d
	}

	res := make([]int, N)

	dfs(0, -1, 1, distanceMap, res)
