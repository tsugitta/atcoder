...
func solve(io *Io, d *Io) {
	...
	sCharToIndices := map[rune][]int{}

	for i, c := range s {
		sCharToIndices[c] = append(sCharToIndices[c], i)
	}
	...
	for _, tc := range t {
		...
		sii := sort.SearchInts(sis, currSi) % len(sis)
		si := sis[sii]
...
func solve2(io *Io, d *Io) {
	...
	for _, tc := range t {
		...
		sii := sort.SearchInts(sis, currSi+1) % len(sis)
		si := sis[sii]
