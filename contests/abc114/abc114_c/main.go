...
func solve(io *Io, d *Io) {
	...
	var dfs func(v int, a, b, c bool) int

	dfs = func(v int, a, b, c bool) int {
		if v > N {
			return 0
		}
		...
		res += dfs(v*10+3, true, b, c)
		res += dfs(v*10+5, a, true, c)
		res += dfs(v*10+7, a, b, true)

		return res
	}

	res := dfs(0, false, false, false)
