...
func solve3(io *Io, d *Io) {
	...
	// dp[i][j]; 1-index で i 文字目、 j 文字目を最後の文字ペアとした時の最大長
	// 当然 S[i] != S[j] (0-index) なら dp[i+1][j+1] == 0
	// また、 dp[0][j], dp[i][0] == 0
	// なお、最初でなく最後としているのは、最初だとすると i, j を小さい方から見ていった時に古い i, j を更新し続ける必要が生まれるため。
	// 最後とすることで、 i, j を古い方から埋めていけば埋めた時点で dp[i][j] が不変となる。
	// 最初とする場合は、文字を後ろから見ていけば問題無い。
	dp := make([][]int, N+1)
	for i := range dp {
		dp[i] = make([]int, N+1)
	}
