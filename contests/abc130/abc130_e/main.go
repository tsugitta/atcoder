...
func solve(io *Io, d *Io) {
	...
	// dp[i][j]: S の i 文字目、 T の j 文字目までを考慮した場合で、その２つを取り入れたケースの個数、の 0 ~ i, 0 ~ j までの総和
	// 和を取っている対象について、末尾に入る数を固定しているため MECE となる。２つが異なっていた場合は 0 となる。
	dp := make([][]int, N+1)
	for i := 0; i <= N; i++ {
		dp[i] = make([]int, M+1)
	}
	...
	for i := 1; i <= N; i++ {
		for j := 1; j <= M; j++ {
			...
			dp[i][j] = (dp[i-1][j] + dp[i][j-1] - dp[i-1][j-1] + ct + MOD) % MOD
		}
	}
