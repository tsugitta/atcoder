...
func solve(io *Io, d *Io) {
	...
	for i := 0; i < M; i++ {
		...
		edges[i] = Edge{
			a,
			b,
			-c, // 最小経路問題に帰着させるためマイナスをかける
		}
	}

	fromToEdgeMaps := make([][]int, N+1)
	toFromEdgeMaps := make([][]int, N+1)

	for _, e := range edges {
		fromToEdgeMaps[e.from] = append(fromToEdgeMaps[e.from], e.to)
		toFromEdgeMaps[e.to] = append(toFromEdgeMaps[e.to], e.from)
	}

	var fillReachableNodeMap func(node int, edges [][]int, reachableMap map[int]bool)
	fillReachableNodeMap = func(node int, edges [][]int, reachableMap map[int]bool) {
		...
		for _, e := range edges[node] {
			fillReachableNodeMap(e, edges, reachableMap)
		}
	}

	reachableFromStartMap := map[int]bool{}
	reachableFromGoalMap := map[int]bool{}

	fillReachableNodeMap(1, fromToEdgeMaps, reachableFromStartMap)
	fillReachableNodeMap(N, toFromEdgeMaps, reachableFromGoalMap)

	reachableMap := map[int]bool{}
	for node, reachableFromStart := range reachableFromStartMap {
		if reachableFromStart && reachableFromGoalMap[node] {
			reachableMap[node] = true
		}
	}
	...
	for i := 1; i <= N; i++ {
		...
		for _, e := range edges {
			if !reachableMap[e.from] || !reachableMap[e.to] {
				continue
			}
