...
func getMode(values []int, ignoreValue *int) (*int, int) {
	var res *int
	resCt := 0
	...
	for i < len(values) {
		...
		if ignoreValue != nil && v == *ignoreValue {
			i++
			continue
		}
...
func solve(io *Io, d *Io) {
	...
	evens := make([]int, (n+1)/2)
	odds := make([]int, n-len(evens))

	for i, v := range vs {
		if i%2 == 0 {
			evens[i/2] = v // 0 -> 0, 2 -> 1, 4 -> 2
		} else {
			odds[i/2] = v // 1 -> 0, 3 -> 1, 5 -> 2
		}
	}
	...
	evenMode, evenModeCt := getMode(evens, nil)
	oddMode, oddModeCt := getMode(odds, nil)

	if *evenMode != *oddMode {
		evensToBeChangedCt := len(evens) - evenModeCt
		oddsToBeChangedCt := len(odds) - oddModeCt

		io.Println(evensToBeChangedCt + oddsToBeChangedCt)
		return
	}

	_, evenSecondModeCt := getMode(evens, evenMode)
	_, oddSecondModeCt := getMode(odds, oddMode)

	res := Min(len(evens)-evenModeCt+len(odds)-oddSecondModeCt, len(odds)-oddModeCt+len(evens)-evenSecondModeCt)
