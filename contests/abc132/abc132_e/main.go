...
func solve(io *Io, d *Io) {
	...
	queue := []point{point{S, 0}}

	for len(queue) > 0 {
		cur := queue[0]
		queue = queue[1:]
		nextIndex := (cur.index + 1) % 3

		for _, nextV := range edges[cur.v] {
			if dist[nextV][nextIndex] == -1 {
				dist[nextV][nextIndex] = dist[cur.v][cur.index] + 1
				queue = append(queue, point{nextV, nextIndex})
			}
		}
	}
