...
func solve(io *Io, d *Io) {
	...
	var okFromPtr func(ptr int) bool
	okFromPtr = func(ptr int) bool {
		if ptr == len(S) {
			return true
		}

		if ptr+5 <= len(S) && S[ptr:ptr+5] == "dream" && okFromPtr(ptr+5) {
			return true
		}

		if ptr+5 <= len(S) && S[ptr:ptr+5] == "erase" && okFromPtr(ptr+5) {
			return true
		}

		if ptr+6 <= len(S) && S[ptr:ptr+6] == "eraser" && okFromPtr(ptr+6) {
			return true
		}

		if ptr+7 <= len(S) && S[ptr:ptr+7] == "dreamer" && okFromPtr(ptr+7) {
			return true
		}
