...
func solve(io *Io, d *Io) {
	...
	M := io.NextInt()
	as := io.NextInts(M)
	matchNumberCanBeUsedMap := map[int]bool{}

	for _, a := range as {
		matchNumber := matchMap[a]
		matchNumberCanBeUsedMap[matchNumber] = true
	}

	// dp[i]: 条件のもとで i 本のマッチを使って作れる整数の最大桁数, 作れない場合は -1
	dp := make([]int, N+1)
	dp[0] = 0
	...
	for i := 1; i <= N; i++ {
		...
		// j: 付け足す桁で使う本数
		for j := minMatch; j <= Min(i, maxMatch); j++ {
			if !matchNumberCanBeUsedMap[j] || dp[i-j] == -1 {
				continue
			}
	...
	for restMatch > 0 {
		// その桁に使える桁数の変わらない最大の整数を出力したい
		var res int

		for _, a := range as {
			matchNumber := matchMap[a]

			if matchNumber > restMatch {
				continue
			}

			// a を用いた場合、残りの本数で最大桁が実現できれば良い
			if dp[restMatch-matchNumber] == dp[restMatch]-1 {
				res = a
				break
			}
		}

		restMatch -= matchMap[res]
		io.Print(res)
	}
