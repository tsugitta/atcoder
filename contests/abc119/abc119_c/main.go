...
// n 本終えた状態, a, b, c それぞれ現状の和
func dfs(n, a, b, c int, input *Input) int {
	if n == input.N {
		if a == 0 || b == 0 || c == 0 {
			return math.MaxInt64 - 10
		}

		return abs(input.A-a) + abs(input.B-b) + abs(input.C-c)
	}

	cur := input.ls[n]
	return Min(
		dfs(n+1, a+cur, b, c, input)+TernaryInt(a > 0, 10, 0),
		dfs(n+1, a, b+cur, c, input)+TernaryInt(b > 0, 10, 0),
		dfs(n+1, a, b, c+cur, input)+TernaryInt(c > 0, 10, 0),
		dfs(n+1, a, b, c, input),
	)
}
