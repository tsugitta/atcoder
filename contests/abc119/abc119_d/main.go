...
func solve(io *Io, d *Io) {
	...
	ss := io.NextInts(A)
	ss = append([]int{-int(10e16)}, ss...)
	ss = append(ss, int(10e16))

	ts := io.NextInts(B)
	ts = append([]int{-int(10e16)}, ts...)
	ts = append(ts, int(10e16))

	for i := 0; i < Q; i++ {
		// a < x < b; a, b in ss
		// c < x < d; c, d in ts
		x := io.NextInt()
		b := sort.SearchInts(ss, x)
		d := sort.SearchInts(ts, x)
		a := b - 1
		c := d - 1

		res := math.MaxInt64

		for _, s := range []int{ss[a], ss[b]} {
			for _, t := range []int{ts[c], ts[d]} {
				res = Min(res, abs(x-s)+abs(s-t), abs(x-t)+abs(t-s))
			}
		}
