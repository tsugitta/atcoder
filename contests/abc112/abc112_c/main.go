...
func solve(io *Io, d *Io) {
	...
	pointToCenterHeight := func(p *Point, cx, cy int) int {
		// NOTE: point.h = H - |X - Cx| - |Y - Cy|
		return p.h + abs(p.x-cx) + abs(p.y-cy)
	}

	for cx := 0; cx <= 100; cx++ {
		for cy := 0; cy <= 100; cy++ {
			h := pointToCenterHeight(hExistPoint, cx, cy)
			ok := true

			for _, p := range ps {
				if p.h != Max(h-abs(p.x-cx)-abs(p.y-cy), 0) {
					ok = false
					break
				}
			}
