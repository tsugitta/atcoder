...
func solve(io *Io, d *Io) {
	...
	// 各桁について、 xor と + は (1, 0), (0, 0) の時は等しい。
	// (1, 1) の時もその桁については等しいが、繰り上がりが起こり、上がった桁が (0, 0), (1, 0), (1, 1)
	// どれであっても等しくなくなる。よって演算される数字でビットの被りがないことが必要十分。
	// a + b - (a ^ b) = 2 * a & b であることを考えるとより明快

	// l を固定した時に条件を満たす最大の r は広義単調増加。よって尺取法が使える。
	// cur: [l, r)
	r := 0
	cur := 0
