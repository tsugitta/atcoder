...
func canBeYear(number int) bool {
	return true
}

func canBeMonth(number int) bool {
...
func solve(io *Io, d *Io) {
	...
	first, _ := strconv.Atoi(S[0:2])
	second, _ := strconv.Atoi(S[2:4])

	yymm := canBeYear(first) && canBeMonth(second)
	mmyy := canBeMonth(first) && canBeYear(second)
