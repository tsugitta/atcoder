...
bool fill_path(ll u, ll p, VL& path) {
...
void fill_ds(ll u, ll p, ll cur_d, VL& ds) {
  chmin(ds[u], cur_d);

  for (ll v : edge_list[u]) {
    if (v == p) continue;
    fill_ds(v, u, cur_d + 1, ds);
  }
}
...
void solve() {
  ...
  limit_v = path[path.size() / 2 - 1];
  ng_v = path[path.size() / 2];
