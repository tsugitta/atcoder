...
func solve(io *Io, d *Io) {
	// 1. a, b に共に 1 である桁が一つもない
	// 2. a + b が L 以下
	// の 2 条件を満たせば良い
	// L 以下の数値について、それを満たす a, b の組を数える
	...
	count1 := 0
	res := 0
	// 「その桁が先頭から見て初めて L が 1 でありながら a + b が 0 である」ような数の個数を求め合計する
	// これは L 以下の数字を MECE に分解している
	// その桁未満の 1 の部分については a, b どちらかが 1 であり、その桁以降については任意
	for i := 0; i < len(chars); i++ {
		restLen := len(chars) - i - 1
		if chars[i] == '1' {
			res += PowM(2, count1) * PowM(3, restLen) % MOD
			res %= MOD
			count1++
		}
	}
