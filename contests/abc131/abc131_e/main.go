...
func solve(io *Io, d *Io) {
	...
	// 2点間の組み合わせは C(N, 2), 辺の数は最小で N-1. 辺の長さは 1 だから、距離を2にできるのは最大で C(N, 2) - (N-1). K がこれより大きければ -1.
	// C(N, 2) - (N-1) = N*(N-1)/2 - (N-1) = (N-1)(N/2 - 1) = (N-1)(N-2)/2 = C(N-1, 2)
	...
	// N-1 本の辺でスターグラフを作ると、 N-1 頂点は互いに距離　2 になり、それらは C(N-1, 2) 個ある。 N-1 個の頂点を結ぶ度に 1 減り、結び方は C(N-1, 2) 通りあるから、 K を 0 にすることもできる。

	edges := make([][]int, 0)

	// スターグラフの辺
	for i := 2; i <= N; i++ {
		edges = append(edges, []int{1, i})
	}
