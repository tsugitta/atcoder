...
func (ws works) Len() int {
	return len(ws)
}

func (ws works) Swap(i, j int) {
	ws[i], ws[j] = ws[j], ws[i]
}

func (ws works) Less(i, j int) bool {
	return ws[i].b < ws[j].b
}
...
func solve(io *Io, d *Io) {
	...
	res := TernaryString(completed, "Yes", "No")
	io.Println(res)
}
