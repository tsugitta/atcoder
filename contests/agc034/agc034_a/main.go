...
// start < x < goal なる x で二連族の壁が一つもない
func getCond1(chars []rune, start, goal int) bool {
	for i := start + 1; i+1 < goal; i++ {
...
// B <= x <= D なる x で前後 1 マスに壁のないものがある （D < C 前提）
func getCond2(chars []rune, B, D int) bool {
	for i := B; i <= D; i++ {
...
func solve(io *Io, d *Io) {
	...
	// C < D なら cond 1 のみで十分。 B を D まで移動させ、 A を C まで移動させる。
	// C > D なら cond 2 も必要。そこで A と B を入れ替える

	cond1 := getCond1(chars, A, C) && getCond1(chars, B, D)
