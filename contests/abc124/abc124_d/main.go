...
// 0 が連続する個数, 1 が連続する個数, 0 が連続する個数..
func getSuccessiveCounts(N int, chars []rune) []int {
	successiveCounts := []int{}
	...
	for i := 1; i < N; i++ {
		...
		if c != currentChar {
			successiveCounts = append(successiveCounts, successiveCount)
			successiveCount = 1
			currentChar = c
			continue
		}
...
func solve(io *Io, d *Io) {
	...
	succCts := getSuccessiveCounts(N, chars)

	accSum := make([]int, len(succCts))
	accSum[0] = succCts[0]

	for i := 1; i < len(succCts); i++ {
		accSum[i] = accSum[i-1] + succCts[i]
	}

	if 2*K >= len(accSum) {
		io.Println(Sum(succCts...))
		return
	}

	max := 0
	firstIsZero := chars[0] == '0'
	for i := 0; 2*K+i <= len(succCts); i++ {
		var partialSum int

		startFromOne := firstIsZero && i%2 == 1 || !firstIsZero && i%2 == 0

		// 1 が先頭のグループを数えれば十分のためスキップ。
		// ただし、文字列の先頭・末尾が 0 であった場合はチェックする必要がある
		if i != 0 && !startFromOne {
			continue
		}

		if i == 0 && !startFromOne {
			partialSum = accSum[2*K-1]
		} else if i == 0 && startFromOne {
			partialSum = accSum[2*K]
		} else {
			partialSum = accSum[Min(2*K+i, len(succCts)-1)] - accSum[i-1]
		}
