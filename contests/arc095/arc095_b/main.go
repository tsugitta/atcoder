...
func solve(io *Io, d *Io) {
	...
	// よって同じ r に対しては 大きい n を取る必要がある
	// また最大の値は r になりえないため、これを n として良い

	ai := Max(as...)

	var aj int
	minDiff := int(1e18)
	// ai が奇数の時は target が ai / 2 と (ai+1) / 2 の２つあるが、
	// (ai+1) / 2 を考慮しないと min にならないケースがあったとしても、
	// ai / 2 との差は 1 しか違わないため、そのケースでは別の値が ai / 2 との差で
	// 同じ min を得ていることがわかる。よって ai / 2 のみを考えれば十分。
	target := ai / 2
