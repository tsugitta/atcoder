...
func (a SortByArg) Len() int      { return len(a) }
func (a SortByArg) Swap(i, j int) { a[i], a[j] = a[j], a[i] }
func (a SortByArg) Less(i, j int) bool {
	atan := func(p P) float64 {
		return math.Atan2(float64(p.y), float64(p.x))
	}

	return atan(a[i]) < atan(a[j])
}
...
func solve(io *Io, d *Io) {
	...
	for f := 0; f < N; f++ {
		for t := 0; t < N; t++ {
			...
			res = MaxF(
				res,
				math.Sqrt(float64(pos.x*pos.x+pos.y*pos.y)),
			)
		}
	}
...
func MaxF(numbers ...float64) float64 {
	max := numbers[0]
	for i, number := range numbers {
		if i == 0 {
			continue
