...
func solve(io *Io, d *Io) {
	...
	// H-1 における各点での個数があれば H における各点での個数も求まる → DP

	// dp[h][w]: 高さ h で w (0..W-1) にたどり着く個数
	dp := make([][]int, H+1)
	for i := range dp {
		dp[i] = make([]int, W)
	}
	...
	// w(0..W-2) と w+1 を結ぶ場合 bit 立てる
	availableBits := []int{}

	for i := 0; i < 1<<uint(W-1); i++ {
		ok := true
		// W-1 個の bit のうち連続して立っているものがあれば ng
		for cur := 0; cur < W-2; cur++ {
			next := cur + 1

			// 下から cur (0..) 桁とそのもう一つ上の桁の比較
			if (i>>uint(cur))&1&(i>>uint(next))&1 == 1 {
				ok = false
			}
		}

		if ok {
			availableBits = append(availableBits, i)
		}
	}

	for i := 1; i <= H; i++ {
		for j := 0; j < W; j++ {
			for _, bit := range availableBits {
				if j >= 1 && bit>>uint(W-j-1)&1 == 1 {
					// j の左側が結ばれているケース
					// bit の左から j-1 個目 (0-index) が立っているかどうかを見る
					// これは右から (W-1)-1 - (j-1) = W-j-1個目 (0-index). bit の要素数は W-1 であることに注意
					dp[i][j] += dp[i-1][j-1]
				} else if j < W-1 && bit>>uint(W-j-2)&1 == 1 {
					// j の右側が結ばれているケース
					// bit の左から j 個目 (0-index) が立っているかどうかを見る
					// これは右から (W-1)-1 - j = W-j-2個目 (0-index)
					dp[i][j] += dp[i-1][j+1]
				} else {
					dp[i][j] += dp[i-1][j]
				}
