...
func solve(io *Io, d *Io) {
	...
	for _, a := range as {
		if a >= 0 {
			pluses = append(pluses, a)
		} else {
			minuses = append(minuses, a)
		}
	}
	...
	// 全てをプラス、マイナスにすることはできない
	// 構成の方法は以下。
	// 1. マイナスにしたいもの、プラスにしたいものをそれぞれ一つずつ m, p として選ぶ
	// 2. m から始め、その他のプラスにしたいもの全てを引く
	// 3. p からそれらを引く。これにより m がマイナス、足した「その他のプラスにしたいもの全て」がプラスになる
	// 4. その他にマイナスにしたいもの全てを引く

	if len(pluses) > 0 && len(minuses) == 0 {
		minPlus := pluses[0]
		pluses = pluses[1:]
		minuses = append(minuses, minPlus)
	} else if len(minuses) > 0 && len(pluses) == 0 {
		maxMinus := minuses[len(minuses)-1]
		minuses = minuses[:len(minuses)-1]
		pluses = append([]int{maxMinus}, pluses...)
	}
	...
	acc := minuses[0]
	for i := 1; i < len(pluses); i++ {
		plus := pluses[i]
		io.Println(acc, plus)
		acc = acc - plus
	}

	io.Println(pluses[0], acc)
	acc = pluses[0] - acc

	for i := 1; i < len(minuses); i++ {
		minus := minuses[i]
		io.Println(acc, minus)
		acc = acc - minus
	}
}
