...
func solve(io *Io, d *Io) {
	...
	for i := 0; i < (1 << 3); i++ {
		...
		for j := 0; j < N; j++ {
			...
			score := 0
			score += TernaryInt(xHigh, c.x, -c.x)
			score += TernaryInt(yHigh, c.y, -c.y)
			score += TernaryInt(zHigh, c.z, -c.z)
