...
type YearAscCities []*City

func (s YearAscCities) Len() int           { return len(s) }
func (s YearAscCities) Swap(i, j int)      { s[i], s[j] = s[j], s[i] }
func (s YearAscCities) Less(i, j int) bool { return s[i].y < s[j].y }

type IdxAscCities []*City

func (s IdxAscCities) Len() int           { return len(s) }
func (s IdxAscCities) Swap(i, j int)      { s[i], s[j] = s[j], s[i] }
func (s IdxAscCities) Less(i, j int) bool { return s[i].idx < s[j].idx }
...
func solve(io *Io, d *Io) {
	...
	sort.Sort(YearAscCities(cities))

	prefCountMap := map[int]int{}

	for i, c := range cities {
		cities[i].order = prefCountMap[c.p] + 1
		prefCountMap[c.p]++
	}

	sort.Sort(IdxAscCities(cities))

	for _, c := range cities {
		io.Println(fmt.Sprintf("%06d%06d", c.p, c.order))
	}
}
