...
func solve(io *Io, d *Io) {
	...
	maxDequeueCount := Min(N, K)
	maxValue := 0

	for dequeueCount := 0; dequeueCount <= maxDequeueCount; dequeueCount++ {
		if dequeueCount == 0 {
			res := Sum(vs...)
		...
		for dequeueFromLeft := 0; dequeueFromLeft <= dequeueCount; dequeueFromLeft++ {
			dequeueFromRight := dequeueCount - dequeueFromLeft
			left := vs[0:dequeueFromLeft]
			right := vs[N-dequeueFromRight : N]
			dequeued := Concat(left, right)
			sort.Ints(dequeued)
			...
			maxEnqueueCount := K - dequeueCount
			enqueueCount := Min(maxEnqueueCount, minusItemCount)
			finalItems := dequeued[enqueueCount:len(dequeued)]
			res := Sum(finalItems...)
...
// Concat returns concatenated slice without mutating original ones
