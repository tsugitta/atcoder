...
func solve(io *Io, d *Io) {
	...
	var hasMultipleSolutionFlag int
	if 2*x == W && 2*y == H {
		hasMultipleSolutionFlag = 1
	} else {
		hasMultipleSolutionFlag = 0
	}

	var halfArea = float64(W*H) / 2
	io.Printfln("%.6f %d", halfArea, hasMultipleSolutionFlag)
}
