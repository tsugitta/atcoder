...
func solve(io *Io, d *Io) {
	...
	// RL となっているすべての L の index を得る
	lIndices := []int{}
	isLIndex := make([]bool, len(chars))
	...
	res := make([]int, len(chars))

	for i := 0; i < len(chars); i++ {
		if chars[i] == 'R' {
			// 右にある最小の L の index
			lIndex := lIndices[sort.SearchInts(lIndices, i)]
			...
			if isLIndex[i] {
				...
				// 左にある最小の L の index
				lIndex := lIndices[sort.SearchInts(lIndices, i)-1]
