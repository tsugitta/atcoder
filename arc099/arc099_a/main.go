...
func solve(io *Io, d *Io) {
	...
	// 1 がどこにあっても、全ての要素をカバーしにいくように埋める必要がある
	// 逆に全ての要素をカバーするのであれば、順番を変えれば 1 を伝播させることができる
	// よって単に全ての要素をカバーするのに必要な回数を考えれば良い

	// 最初の 1 回は K 要素、その後は K-1 要素ずつカバーが増える
	// N-K を K-1 で割り切り上げれば K-1 要素ずつ増やすべき回数が得られる。それに初回の 1 を足す。
	res := ((N-K)+(K-1-1))/(K-1) + 1
	io.Println(res)
}
