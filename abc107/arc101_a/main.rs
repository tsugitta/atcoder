...
pub fn solve<R: BufRead, W: Write>(mut io: Io<R, W>) {
    ...
    // 連続した要素列となるはず。 N-K+1 通りある。
    // 両端のうち近い方の点に向かい、その後もう片方の端へ向かって進む。

    let mut res = 10e10 as usize;

    for from_i in 0..N - K + 1 {
        let to_i = from_i + K - 1;

        let from = xs[from_i];
        let to = xs[to_i];

        let start = min(from.abs(), to.abs());
        let res_for_i = (start + to - from) as usize;
