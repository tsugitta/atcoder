...
func solve(io *Io, d *Io) {
	...
	getNextPoint := func(curr *P) *P {
		reverse := curr.h%2 == 1
		goToNextLine := reverse && curr.w == 0 || !reverse && curr.w == W-1
		...
		w := TernaryInt(reverse, curr.w-1, curr.w+1)
	...
	for i := 0; i < H; i++ {
		for tmpJ := 0; tmpJ < W; tmpJ++ {
			...
			reverse := i%2 == 1
			j := TernaryInt(reverse, W-1-tmpJ, tmpJ)
			curr := P{
				h: i,
				w: j,
			}

			if as[curr.h][curr.w]%2 == 0 {
				continue
			}

			next := getNextPoint(&curr)
