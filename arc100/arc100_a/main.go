...
func solve(io *Io, d *Io) {
	...
	// \sum |c_i - b| の最小化 -> b は {c_n} の median

	sort.Ints(cs)

	// 長さが偶数の時の median を定義通りに求める必要は実は無い
	// ソート済みの配列で中央値を与える 2 要素の間で整数を左右に動かすと、
	// 左側の要素・右側の要素の絶対値の増減が一致するため、合計は変化しない

	// var b int
	// if len(cs)%2 == 0 {
	// 	b = (cs[len(cs)/2] + cs[len(cs)/2-1]) / 2
	// } else {
	// 	b = cs[len(cs)/2]
	// }
