...
func solve(io *Io, d *Io) {
	...
	// 3 つ入れる切断のうち、真ん中を固定することを考える。
	// 左右をさらに 1 度ずつ切断するが、この左右の切断後の 2 つの配列の和が最大値・最小値の候補であるから、
	...
	for dL := 2; dL+1 < N; dL++ {
		...
		// bR: lSum / 2 以上となる最小の as における index
		bR := sort.SearchInts(sums, (lSum+1)/2) - 1

		tmpB := sums[bR+1]
		tmpC := lSum - tmpB

		// B でなく C が lSum / 2 以上となった方が差が小さいケース
		if bR >= 1 && tmpB-tmpC > tmpC-tmpB+2*as[bR] {
			bR--
		}
		...
		dR := sort.SearchInts(sums, lSum+(rSum+1)/2) - 1

		tmpD := sums[dR+1] - lSum
		tmpE := rSum - tmpD

		if dR >= dL+1 && tmpD-tmpE > tmpE-tmpD+2*as[dR] {
			dR--
		}
