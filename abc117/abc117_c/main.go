...
func (ds Diffs) Len() int {
	return len(ds)
}

func (ds Diffs) Swap(i, j int) {
	ds[i], ds[j] = ds[j], ds[i]
}

// Bigger comes first
func (ds Diffs) Less(i, j int) bool {
	return ds[i].diff > ds[j].diff
}
...
func solve(io *Io, d *Io) {
	...
	// Xs をソートし、 M 個に分割。各範囲の最大値・最小値の差の和の最小値が答え。
	// 要素間の差が大きい所から順に分割していけば良い
