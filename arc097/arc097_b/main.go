...
func solve(io *Io, d *Io) {
	...
	// UF の同一グループに含まれるそのグループの index の数は全て p_i = i を実現可能。
	// またそのグループにおける数はそれが最大。よって全てのグループで足し合わせれば求める値となる。
	...
	for i := 0; i < N; i++ {
		...
		if _, alreadyAdded := rootGroupMap[r]; !alreadyAdded {
			rootGroupMap[r] = &Group{}
		}

		g := rootGroupMap[r]
		g.indices = append(g.indices, i)
		g.ps = append(g.ps, ps[i])
	}

	res := 0

	for _, g := range rootGroupMap {
		sort.Ints(g.indices)

		for _, p := range g.ps {
			searchedIndex := sort.SearchInts(g.indices, p)

			if searchedIndex < len(g.indices) && g.indices[searchedIndex] == p {
				res++
			}
		}
	}
