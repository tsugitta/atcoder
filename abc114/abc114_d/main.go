...
func solve(io *Io, d *Io) {
	...
	// 因数の個数が num 以上のものの個数
	factorCt := func(num int) int {
		res := 0

		for _, ct := range factors {
			if ct >= num {
				res++
			}
		}
	...
	// 同じ数が選ばれる 4, 4, 2 のケースは重複が含まれることに注意
	res := factorCt(74) + factorCt(4)*(factorCt(4)-1)*(factorCt(2)-2)/2 + factorCt(24)*(factorCt(2)-1) + factorCt(14)*(factorCt(4)-1)
	io.Println(res)
}
