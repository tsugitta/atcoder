...
func knapsack(maxWeight int, items []item) int {
	// d[i][j]: i (> 0) 番目までのもののみを使った weight j 以下の価値の最大値
	...
	for i := 1; i <= len(items); i++ {
		...
		for j := 0; j <= maxWeight; j++ {
			...
			if item.weight >= 0 && item.weight <= j && dp[i][j-item.weight]+item.value > dp[i][j] {
				dp[i][j] = dp[i][j-item.weight] + item.value
			}
		}
	}
